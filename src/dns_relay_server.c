...
void handle_dns_request(struct Trie *trie, struct Cache *cache, SOCKET sock, struct sockaddr_in clientAddr) {
    ...
    while (1) {
        ...
        if (msg->header->qr == HEADER_QR_QUERY && msg->header->opcode == HEADER_OPCODE_QUERY) // 只处理DNS请求报文
        {
            ...
            log_message("Receving message from %s:%d,the length is %d bytes.\n", inet_ntoa(clientAddr.sin_addr), ntohs(clientAddr.sin_port), len);
            int ip_type = -1;
            ...
            {
                log_message("The default entrance for DNS message");
                if (debug_mode)
                    debug(msg);
                // 将id和客户端绑定,产生新的id
        ...
        } else if (msg->header->qr == HEADER_QR_ANSWER) // 处理从远程DNS服务器返回的DNS响应报文
        {
            ...
            stop_timer(id);
            log_message("Receving message from remote DNS server");
            unsigned char *domain = (unsigned char *)malloc(sizeof(unsigned char) * MAX_DOMAIN_LENGTH);
            ...
            if (type == TYPE_CNAME) {
                ...
                log_message("The send about cname is %d length\n", sendLen);
                free(ipAddr);
                releaseMsg(temp);
                releaseMsg(temp2);
                // 构造新的转发包向服务器进行通信
        ...
        *ip_type = TYPE_AAAA;
        ...
        if (node != 0) {
            ...
            *ip_type = TYPE_A;
            ...
            log_message("Can't find the domain (%s) in cache table or local dictionary tree\n", domain);
            return NULL;
        }
    }
    return ipAddress;
...
void send_dns_response(int sock, Dns_Msg *msg, unsigned char *buf, struct sockaddr_in clientAddr) {
    ...
    log_message("The length of bytestream is %d\n", len);
...
void forward_dns_request(int sock, unsigned char *buf, int len) {
    ...
    if (ret == SOCKET_ERROR)
        log_message("sendto failed with error: %d\n", WSAGetLastError());
    else {
    ...
    while(tmp->front && tmp->front->front){
        if(now - tmp->front->now  <= CLOCKS_PER_SEC * timer / 1000){
            ...
            if(tmp->front->repeated < 3 && now - tmp->front->now  <= CLOCKS_PER_SEC){
                d_tmp = tmp->front;
                log_message("timeout and repeated %d\n", d_tmp->repeated + 1);
