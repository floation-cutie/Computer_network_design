...
void getRRCNAME(Dns_RR *RR, const unsigned char *bytestream, unsigned short *offset, unsigned char *CNAME) {
    ...
    if (RR->type == TYPE_CNAME) {
        unsigned short tempoffset = *offset;
        getName(CNAME, bytestream, &tempoffset);
    }
    (*offset) += RR->rdlength;
}
void getCNAME(const unsigned char *bytestream, unsigned short *offset, unsigned char *cname) {
    ...
    *offset = 12;
    ...
    for (int i = 0; i < total_length; i++) {
        ...
        RRs_tail = current;
        getRRCNAME(current, bytestream, offset, cname);
    }
    releaseMsg(msg);
}
// 从字节流中提取RR的内容
...
Dns_Msg *create_dns_msg(const unsigned char *bytestream, unsigned short *offset, unsigned char *cname) {
    Dns_Msg *msg = (Dns_Msg *)malloc(sizeof(Dns_Msg));
    if (!msg) {
        puts("Allocate memory failed");
        exit(1);
    ...
    *offset = 12;
    ...
    for (int i = 0; i < msg->header->qdcount; i++) {
        ...
        question_tail = current;
        current->qname = (unsigned char *)malloc(sizeof(unsigned char) * UDP_MAX);
        if (!current->qname) {
            puts("Allocate memory failed");
            exit(1);
        }
        getName(current->qname, bytestream, offset);
        memcpy(current->qname, cname, strlen(cname) + 1);
        current->qtype = ntohs(*(unsigned short *)(bytestream + *offset));
        (*offset) += 2;
        current->qclass = ntohs(*(unsigned short *)(bytestream + *offset));
        (*offset) += 2;
    }
    if (debug_mode)
        printf("After question, the offset is %d\n", *offset);
    // 转换answer、authority、 additional部分
    unsigned short total_length = msg->header->ancount + msg->header->nscount + msg->header->arcount;
    msg->RRs = NULL;
    Dns_RR *RRs_tail = NULL;
    ...
    if (debug_mode)
        printf("After RR, the offset is %d\n", *offset);
    return msg;
}
